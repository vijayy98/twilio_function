...
express()
  ...
  .post("/saveUser",(req,res)=> saveUser(req,res))
...
const callerUserId = async (phone) => {
  try {
    console.log("check user exist in DB ->", phone);
  ...
  myVoiceIt.checkUserExists(
    ...
    async (jsonResponse) => {
      ...
      if (jsonResponse.exists === true) {
        ...
        console.log("user already exist in our DB -> lets get the User name -> ")
        ...
        // res.status(200)
        // res.send({ redirect:"getUser",phone: phone,userId: userId});
      } else {
        // Create a new user for new number
        console.log("no user in voiceIT --->");
        twiml.redirect("/saveUser");
        res.type("application/json")
        res.status(200);
        res.send({ redirect:"getUser",phone: phone,userId: userId});
       /* myVoiceIt.createUser(async (jsonResponse) => {
          ...
          try {
            var finalName = usersName[
              Math.floor(Math.random() * Math.floor(usersName.length))
            ]
              .concat(" ")
              .concat(makeid());
...
const saveUser =async (req,res) => {
  ...
  console.log("user data from angular --->", name);
  console.log("user data from angular phone--->", phone);
  ...
  myVoiceIt.voiceVerificationByUrl(
    ...
    async (jsonResponse) => {
      ...
      if (jsonResponse.responseCode == "SUCC") {
        ...
        speak(
          twiml,
          "Thank you for calling voice its voice biometrics demo. Have a nice day!"
        );
        //Hang up
      } else if (numTries > 2) {
        //3 attempts failed
        speak(
          twiml,
          "Too many failed attempts. Please call back and select option 1 to re enroll and verify again."
        );
      } else {
        switch (jsonResponse.responseCode) {
          case "STTF":
            speak(
              twiml,
              "Verification failed. It seems you may not have said your enrolled phrase. Please try again."
            );
            numTries = numTries + 1;
            twiml.redirect("/verify");
            break;
          ...
          default:
            speak(
              twiml,
              "Something went wrong. Your verification did not pass, please try again."
            );
            numTries = numTries + 1;
            twiml.redirect("/verify");
        }
